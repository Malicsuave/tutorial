...
$db_first_name = $db_middle_name = $db_last_name = $db_gender = $db_preffix = $db_seven_digit = $db_email = $db_password = '';
...
if (mysqli_num_rows($get_record) > 0) {
    ...
    // Debugging step: Ensure the values are retrieved correctly
    // Remove or comment out this line after confirming the output
    
} 

// Initialize new variables and error messages
...
if(isset($_POST["btnUpdate"])){
    ...
    if(empty($_POST["new_first_name"])){
        $new_first_nameErr = "This field must no be empty!";
    }else{
    ...
    if(empty($_POST["new_middle_name"])){
        $new_middle_nameErr = "This field must no be empty!";
    }else{
    ...
    if(empty($_POST["new_last_name"])){
        $new_last_nameErr = "This field must no be empty!";
    }else{
    ...
    if(empty($_POST["new_seven_digit"])){
        $new_seven_digitErr = "This field must no be empty!";
    }else{
    ...
    if(empty($_POST["new_email"])){
        $new_emailErr = "This field must no be empty!";
    }else{
...
<center>
    ...
    <form method="POST">
        <table border="0" width="50%">
            <tr>
                <td>
                    <!-- Populate the input with the first name from the database -->
                    <input type="text" name="new_first_name" value="<?php echo htmlspecialchars($db_first_name); ?>"> 
                    <span class="error"><?php echo $new_first_nameErr; ?></span>
                </td>
            </tr>
            <tr>
                <td>
                    <!-- Populate the input with the middle name from the database -->
                    <input type="text" name="new_middle_name" value="<?php echo htmlspecialchars($db_middle_name); ?>">
            ...
            <tr>
                <td>
                    <!-- Populate the input with the middle name from the database -->
                    <input type="text" name="new_last_name" value="<?php echo htmlspecialchars($db_last_name); ?>">
